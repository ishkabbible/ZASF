...
void waveshapesmps(int n,REALTYPE *smps,unsigned char type,unsigned char drive)
{
    ...
    case 3:
        ...
        for (i=0;i<n;i++) {
            smps[i]*=ws;
            if (fabs(smps[i])<1.0) {
                smps[i]=(smps[i]-pow(smps[i],3.0f))*3.0f;
                if (ws<1.0) smps[i]/=ws;
            } else smps[i]=0.0;
        };
        break;
    ...
    case 13:
        ...
        break;
    case 14:
        ws=pow(ws,5.0f)*80.0+0.0001;//sigmoid
        if (ws>10.0) tmpv=0.5;
...
Distorsion::Distorsion(const int &insertion_,REALTYPE *efxoutl_,REALTYPE *efxoutr_,ParamID *parID_)
