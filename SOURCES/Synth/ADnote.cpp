...
ADnote::ADnote(ADnoteParameters *pars,Controller *ctl_,REALTYPE freq,REALTYPE velocity,int portamento_,int midinote_,bool besilent)
{
    ...
    for (int nvoice=0;nvoice<NUM_VOICES;nvoice++) {
        ...
        default: {   //unison for more than 2 subvoices
            REALTYPE *unison_values;
			unison_values = new REALTYPE [unison];
            ...
            for(int k = 0; k < unison; k++) {
                ...
                unison_base_freq_rap[nvoice][k] =
                    pow(2.0f, (unison_spread * unison_values[k]) / 1200);
            }
        };
        }
...
void ADnote::KillNote()
{
    ...
    for (nvoice=0;nvoice<NUM_VOICES;nvoice++) {
		...
		////***VDX
		//if (partparams)
		//	partparams->VoicePar[nvoice].OscilSmp->b_changed = 0;
...
void ADnote::update_parameters_realtime()
...
//by VDX
{
	if (partparams)
	{
		...
		//Global Stuff
		if (partparams->b_changed)
		{
			NoteGlobalPar.Volume=4.0*pow(0.1,3.0*(1.0-partparams->GlobalPar.PVolume/96.0))//-60 dB .. 0 dB
                         *VelF(velocity,partparams->GlobalPar.PAmpVelocityScaleFunction);//velocity sensing

			 if (partparams->GlobalPar.PPanning==0) NoteGlobalPar.Panning=RND;
			 else NoteGlobalPar.Panning=partparams->GlobalPar.PPanning/128.0;			

			partparams->b_changed --;
		};

	   
	   //Frequency LFO
	   if (partparams->GlobalPar.FreqLfo)
	    if (partparams->GlobalPar.FreqLfo->b_changed)
	      {
	       NoteGlobalPar.FreqLfo->init(partparams->GlobalPar.FreqLfo,basefreq);
		   partparams->GlobalPar.FreqLfo->b_changed--;
	      };


	   //Amplitude LFO
	   if (partparams->GlobalPar.AmpLfo)
		if (partparams->GlobalPar.AmpLfo->b_changed)
		{
			NoteGlobalPar.AmpLfo->init(partparams->GlobalPar.AmpLfo,basefreq);
			partparams->GlobalPar.AmpLfo->b_changed--;
		};


	   //Filter LFO
	   if (partparams->GlobalPar.FilterLfo)
		   if (partparams->GlobalPar.FilterLfo->b_changed)
		   {
			   NoteGlobalPar.FilterLfo->init(partparams->GlobalPar.FilterLfo,basefreq);
			   partparams->GlobalPar.FilterLfo->b_changed--;
		   }



		//Filter parameters
        if (partparams->GlobalPar.GlobalFilter)
			if (partparams->GlobalPar.GlobalFilter->b_changed)
			{

				//update central frequency
				NoteGlobalPar.FilterCenterPitch = partparams->GlobalPar.GlobalFilter->getfreq() +
					partparams->GlobalPar.PFilterVelocityScale/127.0*6.0* 
					(VelF(velocity,partparams->GlobalPar.PFilterVelocityScaleFunction)-1);
				...
				//update gain
				NoteGlobalPar.GlobalFilterL->update_gain(partparams->GlobalPar.GlobalFilter);
				NoteGlobalPar.GlobalFilterR->update_gain(partparams->GlobalPar.GlobalFilter);
				...
				partparams->GlobalPar.GlobalFilter->b_changed--;			
			}




	   //Envelopes don't need to be updated in realtime, now do they??





		//ADsynth LOCAL (voice) Parameters of the Instrument
	

		for (int nvoice=0; nvoice<NUM_VOICES; nvoice++) {
        
		//Global Stuff
		if (partparams->VoicePar[nvoice].b_changed)
		{
			NoteVoicePar[nvoice].Volume=pow(0.1,3.0*(1.0-partparams->VoicePar[nvoice].PVolume/127.0)) // -60 dB .. 0 dB
                                    *VelF(velocity,partparams->VoicePar[nvoice].PAmpVelocityScaleFunction);//velocity
			
			 if (partparams->VoicePar[nvoice].PPanning==0) NoteVoicePar[nvoice].Panning=RND;
			 else NoteVoicePar[nvoice].Panning=partparams->VoicePar[nvoice].PPanning/128.0;			

			partparams->VoicePar[nvoice].b_changed --;
		};

		
		//Frequency LFO
		if (partparams->VoicePar[nvoice].FreqLfo)
			if (partparams->VoicePar[nvoice].FreqLfo->b_changed)
			{
				//*((int*)0) = 98;  //a veeewy vewy nice way to add a breakpoint (crash za system hahahaha)
				if (NoteVoicePar[nvoice].FreqLfo)
					NoteVoicePar[nvoice].FreqLfo->init(partparams->VoicePar[nvoice].FreqLfo,basefreq);
				partparams->VoicePar[nvoice].FreqLfo->b_changed --;
			};


		//Amplitude LFO
		if (partparams->VoicePar[nvoice].AmpLfo)
			if (partparams->VoicePar[nvoice].AmpLfo->b_changed)
			{
				if (NoteVoicePar[nvoice].AmpLfo)
					NoteVoicePar[nvoice].AmpLfo->init(partparams->VoicePar[nvoice].AmpLfo,basefreq);
				partparams->VoicePar[nvoice].AmpLfo->b_changed --;
			};


	   //Filter LFO
	   if (partparams->VoicePar[nvoice].FilterLfo)
		   if (partparams->VoicePar[nvoice].FilterLfo->b_changed)
		   {
			   if (NoteVoicePar[nvoice].FilterLfo)
				   NoteVoicePar[nvoice].FilterLfo->init(partparams->VoicePar[nvoice].FilterLfo,basefreq);
			   partparams->VoicePar[nvoice].FilterLfo->b_changed --;
		   };
		
		
		//Filter parameters
		if (partparams->VoicePar[nvoice].VoiceFilter)
			if (partparams->VoicePar[nvoice].VoiceFilter->b_changed)
			{
				//update central frequency
				NoteVoicePar[nvoice].FilterCenterPitch = partparams->VoicePar[nvoice].VoiceFilter->getfreq();
				...
				if (NoteVoicePar[nvoice].VoiceFilterL) {
					...
					//update Q				
					NoteVoicePar[nvoice].VoiceFilterL->setq(partparams->VoicePar[nvoice].VoiceFilter->getq());
					NoteVoicePar[nvoice].VoiceFilterR->setq(partparams->VoicePar[nvoice].VoiceFilter->getq());

					//update gain
					NoteVoicePar[nvoice].VoiceFilterL->update_gain(partparams->VoicePar[nvoice].VoiceFilter);
					NoteVoicePar[nvoice].VoiceFilterR->update_gain(partparams->VoicePar[nvoice].VoiceFilter);
				}
				
				partparams->VoicePar[nvoice].VoiceFilter->b_changed --;

			};

		//Unison
		if (partparams->VoicePar[nvoice].b_unison_changed) {
					
			unison_stereo_spread[nvoice] = partparams->VoicePar[nvoice].Unison_stereo_spread / 127.0f;


			partparams->VoicePar[nvoice].b_unison_changed--;
			//UNFINISHED STUFF
		}
       ...
       /* oscposhi[nvoice]=0;
		...
		if(pars->VoicePar[nvoice].OscilSmp->b_changed)
		{
			//pars->VoicePar[nvoice].OscilSmp->oscilprepared= 0;
			
			//while (pars->VoicePar[nvoice].OscilSmp->oscilprepared==0)

			for (j=0; j<NUM_VOICES; j++){
				if (pars->VoicePar[j].Pextoscil==nvoice)
					pars->VoicePar[j].OscilSmp->b_changed = 10;
			}
		
			if (pars->VoicePar[nvoice].Pextoscil!=-1) {
				vc=pars->VoicePar[nvoice].Pextoscil;
				//pars->VoicePar[vc].OscilSmp->b_changed =1;
		  ...
		  //***
		  //vc = nvoice;
		  if (NoteVoicePar[nvoice].OscilSmp == 0) return;
		  /*if (pars->VoicePar[nvoice].OscilSmp->b_changed >9) {
			
              pars->VoicePar[nvoice].OscilSmp->oscilpreparing = 0;
			  pars->VoicePar[nvoice].OscilSmp->prepare();
			  pars->VoicePar[nvoice].OscilSmp->oscilpreparing = 1;			
		  };*/
		
		  //pars->VoicePar[nvoice].OscilSmp->oscilpreparing = 1;
			ready = 0;
		  if (pars->VoicePar[nvoice].OscilSmp->b_changed <5) {
			  
			  pars->VoicePar[nvoice].OscilSmp->oscilpreparing = 0;
			  pars->VoicePar[nvoice].OscilSmp->prepare();
			  pars->VoicePar[nvoice].OscilSmp->oscilpreparing = 1;
			  
			  pars->VoicePar[vc].OscilSmp->get(NoteVoicePar[nvoice].OscilSmp,getvoicebasefreq(nvoice),
                        pars->VoicePar[nvoice].Presonance);
		  ...
		  //if (pars->VoicePar[nvoice].OscilSmp->b_changed==0) 
		  ready = 1;
         ...
         // oscposhi[nvoice]+=(int)((pars->VoicePar[nvoice].Poscilphase-64.0)/128.0*OSCIL_SIZE+OSCIL_SIZE*4);
		  ...
		  //do_fadein = 1;
		  pars->VoicePar[nvoice].OscilSmp->b_changed--;
		  //if (pars->VoicePar[nvoice].OscilSmp->b_changed == 0) pars->VoicePar[nvoice].OscilSmp->oscilpreparing = 0;
		}
	...
	};// checked if partparams pointer is valid
}
...
void ADnote::initparameters()
{
        ...
        if ((NoteVoicePar[nvoice].FMEnabled!=NONE)&&(NoteVoicePar[nvoice].FMVoice<0)) {
            ...
            for(int k = 0; k < unison_size[nvoice]; k++)
                oscposhiFM[nvoice][k] = (oscposhi[nvoice][k]
                     + partparams->VoicePar[vc].FMSmp->get(NoteVoicePar[nvoice].FMSmp, tmp)) % OSCIL_SIZE;
    ...
    for (nvoice=0;nvoice<NUM_VOICES;nvoice++) {
        ...
        if (NoteVoicePar[nvoice].VoiceOut!=NULL) 
			//for (i=0;i<SOUND_BUFFER_SIZE;i++) NoteVoicePar[nvoice].VoiceOut[i]=0.0;
			memset(NoteVoicePar[nvoice].VoiceOut, 0, SOUND_BUFFER_SIZE * sizeof(REALTYPE));
    };
};
...
REALTYPE ADnote::getvoicebasefreq(int nvoice)
{
    ...
    if (NoteVoicePar[nvoice].fixedfreq==0) return(this->basefreq*pow(2,detune/12.0));
        ...
        if (fixedfreqET!=0) {//if the frequency varies according the keyboard note
            ...
            if (fixedfreqET<=64) fixedfreq*=pow(2.0f,tmp);
            else fixedfreq*=pow(3.0f,tmp);
        };
        return(fixedfreq*pow(2.0f,(float)(detune/12.0)));
    };
};
...
void ADnote::computecurrentparameters()
{
	...
	//twiddle with global adnote filter frequency and Q
	...
	//******VDX
	//update other UI parameters in realtime
	update_parameters_realtime();
...
int ADnote::noteout(REALTYPE *outl,REALTYPE *outr)
{
	...
	for (nvoice=0;nvoice<NUM_VOICES;nvoice++) {
        ...
        REALTYPE unison_amplitude = 1.0 / sqrt((double)unison_size[nvoice]); //reduce the amplitude for large unison sizes
        // Amplitude
        ...
        if (NoteVoicePar[nvoice].AmpEnvelope!=NULL) {
			if(NoteVoicePar[nvoice].AmpEnvelope->finished() != 0) {
                ...
                if(stereo)
                for (i=0;i<SOUND_BUFFER_SIZE;i++)
                    tmpwaver[i]*=1.0-(REALTYPE)i/(REALTYPE)SOUND_BUFFER_SIZE;
			}
