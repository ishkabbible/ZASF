...
OscilGen::OscilGen(FFTwrapper *fft_,Resonance *res_,ParamID *parID_):Presets()
{
...
void OscilGen::convert2sine(int magtype)
{
    ...
    phase[0]=0;
    for (int i=0;i<MAX_AD_HARMONICS;i++) {
        mag[i]=sqrt(pow(freqs.s[i+1],2.0f)+pow(freqs.c[i+1],2.0f));
...
REALTYPE OscilGen::basefunc_power(REALTYPE x,REALTYPE a)
{
    ...
    return(pow(x,exp((a-0.5f)*10.0f))*2.0f-1.0f);
};
...
REALTYPE OscilGen::basefunc_abssine(REALTYPE x,REALTYPE a)
{
    ...
    return(sin(pow(x,exp((a-0.5f)*5.0f))*PI)*2.0f-1.0f);
};

REALTYPE OscilGen::basefunc_pulsesine(REALTYPE x,REALTYPE a)
{
    if (a<0.00001) a=0.00001;
    x=(fmod(x,1)-0.5)*exp((a-0.5)*log(128.0f));
    if (x<-0.5) x=-0.5;
    else if (x>0.5) x=0.5;
    x=sin(x*PI*2.0);
    return(x);
...
//    printf("%.5f %.5f\n",basefuncmodulationpar1,basefuncmodulationpar3);
    ...
    for (i=0;i<OSCIL_SIZE;i++) {
        ...
        case 2:
            ...
            break;
        case 3:
            t=t+pow((float)((1.0f-cos((t+basefuncmodulationpar2)*2.0f*PI))*0.5f),basefuncmodulationpar3)*basefuncmodulationpar1;//power
            break;
        };
...
void OscilGen::oscilfilter()
{
    ...
    for (int i=1;i<OSCIL_SIZE/2;i++) {
        ...
        case 1:
            ...
            if (gain<tmp) gain=pow(gain,10.0f)/pow(tmp,9.0f);
            break;
        case 2:
            gain=1.0-pow(1.0-par*par,i+1);//hp1
            gain=pow(gain,par2*2.0f+0.1f);
            break;
        case 3:
        ...
        case 5:
            ...
            gain=pow(gain,par2*par2*3.9f+0.1f);
            break;
        case 6:
            tmp=pow(par2,0.33f);
...
void OscilGen::modulation()
{
    ...
    for (i=0;i<OSCIL_SIZE;i++) {
        ...
        case 2:
            ...
            break;
        case 3:
            t=t+pow((float)((1.0f-cos((t+modulationpar2)*2.0f*PI))*0.5f),modulationpar3)*modulationpar1;//power
            break;
        };
...
void OscilGen::spectrumadjust()
{
    ...
    case 1:
        par=1.0-par*2.0;
        if (par>=0.0) par=pow(5.0f,par);
        else par=pow(8.0f,par);
        break;
    case 2:
        par=pow(10.0f,(1.0f-par)*3.0f)*0.25f;
        break;
    case 3:
        par=pow(10.0f,(1.0f-par)*3.0f)*0.25f;
        break;
    ...
    REALTYPE max=0.0;
    for (int i=0;i<OSCIL_SIZE/2;i++) {
        REALTYPE tmp=pow(oscilFFTfreqs.c[i],2.0f)+pow(oscilFFTfreqs.s[i],2.0f);
        if (max<tmp) max=tmp;
    };
    ...
    for (int i=0;i<OSCIL_SIZE/2;i++) {
        REALTYPE mag=sqrt(pow(oscilFFTfreqs.s[i],2.0f)+pow(oscilFFTfreqs.c[i],2.0f))/max;
...
short int OscilGen::get(REALTYPE *smps,REALTYPE freqHz,int resonance)
{
    ...
    if ((oldbasepar!=Pbasefuncpar)||(oldbasefunc!=Pcurrentbasefunc)||(oldhmagtype!=Phmagtype)
            ||(oldwaveshaping!=Pwaveshaping)||(oldwaveshapingfunction!=Pwaveshapingfunction)) 
			oscilprepared=0;
