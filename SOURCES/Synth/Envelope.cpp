...
extern float minEnvDB;		//global setting defined in main.c
...
Envelope::Envelope(EnvelopeParams *envpars,REALTYPE basefreq)
{
    ...
    for (i=0;i<MAX_ENVELOPE_POINTS;i++) {
        ...
        switch (mode) {
        case 2:
            //envval[i]=(1.0-envpars->Penvval[i]/127.0)*MIN_ENVELOPE_DB;		//VDX disabled
			envval[i]=(1.0-envpars->Penvval[i]/127.0)*minEnvDB;
            break;
        case 3:
...
REALTYPE Envelope::envout_dB()
{
    ...
    if ((currentpoint==1)&&((keyreleased==0)||(forcedrelase==0))) {//first point is always lineary interpolated
        ...
        else envoutval=minEnvDB;		//used to be -40
