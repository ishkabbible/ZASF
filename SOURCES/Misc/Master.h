...
class VSTSynth;  //include statement in cpp to avoid recursivity
class MidiRL;
...
public:
	...
	//function pointer to callback updateDisplay to host
	//bool (*cb_updateDisplay)(void);
	VSTSynth* theVST;
	...
	//void setGui(MasterUI* gui);
	int b_turbo;
	int b_multiCh;
	long b_writingAutomation;
    ...
    //void GetAudioOutSamples(int nsamples,int samplerate,float *outl,float *outr);
	void GetAudioOutSamples(int nsamples,int samplerate,float **);
	int SetAudioInSamples(int input, int nsamples, int samplerate, float *inl, float *inr);
	int AudioInNoteOut(int input,REALTYPE *outl,REALTYPE *outr);
	...
	//input streams
	//unsigned char PExt1_en;
	//unsigned char PExt2_en;
	//unsigned char PExt3_en;
	//unsigned char PExt4_en;
	int PExt_en [4];
