...
Echo::Echo(const int & insertion_,REALTYPE *const efxoutl_,REALTYPE *const efxoutr_, ParamID *parID_)
        : Effect(insertion_,efxoutl_,efxoutr_,NULL,0,parID_),
...
void Echo::initdelays_rl()
{
    ...
    newdl=(int)(1+delay.getiVal()*SAMPLE_RATE-lrdelay);
    if (dl<1) dl=1;
    newdr=(int)(1+delay.getiVal()*SAMPLE_RATE+lrdelay);
    if (dr<1) dr=1;


	//find a way to resize the buffers without erasing contents

	delaySample.left().resize(newdl);
	delaySample.right().resize(newdr);
...
//void Echo::out(const Stereo<AuSample> & input){ //ideal
	...
	//this undescribable line copies the smps into input (its internal buffers)
